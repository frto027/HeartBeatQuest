...
private:
    jmethodID
        ...
        ;
public:
    BleReader(jobject SomeClassLoader, jmethodID LoadClassMethod, JNIEnv * env) : JavaSingleClassObject(SomeClassLoader, LoadClassMethod, env, "top.zxff.nativeblereader.BleReader") {
        nativeMethods.emplace_back("OnDeviceData", "(Ljava/lang/String;IJ)V", reinterpret_cast<void*>(OnDeviceData));
        nativeMethods.emplace_back("InformNativeDevice", "(Ljava/lang/String;[B)Z", reinterpret_cast<void*>(InformNativeDevice));
        nativeMethods.emplace_back("OnEnergyReset","()V",reinterpret_cast<void*>(OnEnergyReset));
        nativeMethods.emplace_back("OnAutoConnectStatusChanged","(Z)V",reinterpret_cast<void*>(OnAutoConnectStatusChanged));
    ...
    void autoConnectSetPattern(const std::string & macAddr, const std::string& devName){
        env->CallVoidMethod(ThisObj, m_autoConnectSetPattern, env->NewStringUTF(macAddr.c_str()), env->NewStringUTF(devName.c_str()));
        CheckException();
    }
    ...
    static JNIEXPORT void JNICALL OnDeviceData(JNIEnv *env, jobject thiz, jstring macAddr, jint heartRate, jlong energy);

...
private:
    jmethodID
        ...
        ;
public:
    MDnsHelper(jobject SomeClassLoader, jmethodID LoadClassMethod, JNIEnv * env) : JavaSingleClassObject(SomeClassLoader, LoadClassMethod, env, "top.zxff.nativeblereader.MDnsHelper") {        
    ...
    void SetMDnsName(std::string name, jint port, std::string type){
        env->CallVoidMethod(ThisObj, m_SetMdnsName, env->NewStringUTF(name.c_str()), port, env->NewStringUTF(type.c_str()));
        CheckException();
    }
